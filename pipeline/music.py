        ...
        if len(by_artist) > 1:
            ...
            for a, cs in rows[:MAX_CANDIDATES]:
                ...
                candidates.append(Candidate(
                    label=f"{a} — {c0['title']}",
                    disambiguator=_candidate_when(cs, earliest, em)
...
def _candidate_when(cs: list[dict], earliest: Optional[str], em: Optional[Emitter] = None) -> str:
    """
    ...
    The dated relation usually lives on the canonical entity, not on the
    earliest-release one, so up to two of the artist's entities are probed:
    highest search score first, then earliest release. Cached lookups; the
    fallback wording names its own weakness, so "recorded 1928" vs
    "earliest release on file 1992" reads without explanation.
    """
    by_score = sorted(cs, key=lambda c: (-(c.get("score") or 0), c["date"] or "9999"))
    by_release = sorted(cs, key=lambda c: c["date"] or "9999")
    probed = set()
    for c in (by_score[0], by_release[0]):
        if c["mbid"] in probed:
            continue
        probed.add(c["mbid"])
        f = mb.recording_works(c["mbid"])
...
def _suggestion_candidates(hits: list[dict], limit: int = 5) -> list[Candidate]:
    ...
    for a, cs in rows[:limit]:
        ...
        out.append(Candidate(
            label=f"{a} — {c0['title']}",
            disambiguator=_candidate_when(cs, earliest)
                          + f"; {len(cs)} recording entit{'y' if len(cs) == 1 else 'ies'}",
            identifiers=[Identifier(scheme="musicbrainz_recording", value=c0["mbid"],
                                    layer_id=RECORDING, confidence=Confidence.LOW)],
            likelihood=Confidence.LOW,
