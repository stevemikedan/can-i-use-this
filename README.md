...
- **Answer and re-run**: every answerable question takes the fact it names — the renewal boolean, the composition's publication year, the recording's first-release year, the last surviving writer's death year — with an optional source attestation, and the verdict re-runs under the confidence policy above. The collapsed question names the path, so an answerable question reads as answerable before any click; leaving it unanswered keeps it open.
